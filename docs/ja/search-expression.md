...
パーサーにはJavaScriptとC++の両方の実装があります。`client-factory`からインポートすると、ライブラリは自動的にネイティブC++実装を使用し、利用できない場合はJavaScriptにフォールバックします。

```typescript
import { simplifySearchExpression } from 'mygramdb-client';

// ネイティブC++が利用可能なら使用、なければJavaScript
...
**高性能なネイティブ C++ パーサー** - Web形式の検索式を構造化された用語に変換します。この関数は、利用可能な場合はネイティブ C++ 実装を自動的に使用し、ネイティブバインディングがビルドされていない場合は JavaScript パーサーにフォールバックします。

**パラメータ:**
- `expression` (string) - Web形式の検索式

**戻り値:** 解析された用語を含むオブジェクト

**例外:** 式が無効な場合、または正の用語がない場合は Error をスロー

**パフォーマンス:** ネイティブ実装は、特に高スループットアプリケーションにおいて、複雑な式の解析で大幅に優れたパフォーマンスを提供します。
